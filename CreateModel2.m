...
function model=CreateModel2()
    ...
    mu1 = [MAP_SIZE/2 MAP_SIZE/3];  % Mean of first distribution (Potential target position)
    Sigma1 = MAP_SIZE*[0.11 0;0 0.11]; % Covarian of the distribution
    ...
    Pmap = 0.5*(F1 + F2); % Standardise the map with two target info sources
